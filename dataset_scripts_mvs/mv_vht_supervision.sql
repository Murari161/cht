...
SELECT
    ...
    doc #>> '{contact,parent,parent,_id}'::text[] AS district_id,
    doc #>> '{contact,parent,parent,parent,_id}'::text[] AS region_id,
    ...
    doc #>> '{fields,vht_supervised}'::text[] AS vht_supervised,
    doc #>> '{fields,group_vht_supervision_status,vht_supervision_status}'::text[] AS vht_supervision_status,

    -- Last column for tracking refresh
    CURRENT_TIMESTAMP AS last_refresh_date

FROM dwh.cht_data couchdb
WHERE (doc ->> 'form'::text) = 'vht_supervision'::text
  AND is_current
WITH DATA;

-- Indexes
CREATE INDEX mv_vht_supervision_reported
    ON cht.mv_vht_supervision USING btree (reported);

CREATE INDEX mv_vht_supervision_chw_id
    ON cht.mv_vht_supervision USING btree (chw_id);