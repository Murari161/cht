...
FROM (
    SELECT
        ...
        LOWER(TRIM(model_hh_have_drying_lines)) AS model_hh_have_drying_lines,
        LOWER(TRIM(model_hh_have_animal_house)) AS model_hh_have_animal_house,
        LOWER(TRIM(model_hh_have_food_storage_access)) AS model_hh_have_food_storage_access,
        LOWER(TRIM(model_hh_compound_well_maintained)) AS model_hh_compound_well_maintained,
        LOWER(TRIM(model_hh_have_vermin_rodent)) AS model_hh_have_vermin_rodent,
        LOWER(TRIM(is_model_household)) AS is_model_household
    FROM cht.mv_wash_report
) src
CROSS JOIN LATERAL (
    VALUES
        -- needs_signoff (true/false) - binary
        ('needs_signoff - True', CASE WHEN src.needs_signoff = 'true' THEN 1 ELSE 0 END),
        ('needs_signoff - False', CASE WHEN src.needs_signoff = 'false' THEN 1 ELSE 0 END),
        ...
        -- model_hh_have_drying_lines (yes/no) - binary
        ('model_hh_have_drying_lines - Yes', CASE WHEN src.model_hh_have_drying_lines = 'yes' THEN 1 ELSE 0 END),
        ('model_hh_have_drying_lines - No', CASE WHEN src.model_hh_have_drying_lines = 'no' THEN 1 ELSE 0 END),

        -- model_hh_have_animal_house (yes/no) - binary
        ('model_hh_have_animal_house - Yes', CASE WHEN src.model_hh_have_animal_house = 'yes' THEN 1 ELSE 0 END),
        ('model_hh_have_animal_house - No', CASE WHEN src.model_hh_have_animal_house = 'no' THEN 1 ELSE 0 END),

        -- model_hh_have_food_storage_access (yes/no) - binary
        ('model_hh_have_food_storage_access - Yes', CASE WHEN src.model_hh_have_food_storage_access = 'yes' THEN 1 ELSE 0 END),
        ('model_hh_have_food_storage_access - No', CASE WHEN src.model_hh_have_food_storage_access = 'no' THEN 1 ELSE 0 END),

        -- model_hh_compound_well_maintained (yes/no) - binary
        ('model_hh_compound_well_maintained - Yes', CASE WHEN src.model_hh_compound_well_maintained = 'yes' THEN 1 ELSE 0 END),
        ('model_hh_compound_well_maintained - No', CASE WHEN src.model_hh_compound_well_maintained = 'no' THEN 1 ELSE 0 END),

        -- model_hh_have_vermin_rodent (yes/no) - binary
        ('model_hh_have_vermin_rodent - Yes', CASE WHEN src.model_hh_have_vermin_rodent = 'yes' THEN 1 ELSE 0 END),
        ('model_hh_have_vermin_rodent - No', CASE WHEN src.model_hh_have_vermin_rodent = 'no' THEN 1 ELSE 0 END),
