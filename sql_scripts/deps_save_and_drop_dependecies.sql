-- DROP FUNCTION cht.deps_save_and_drop_dependencies(name, name);

CREATE OR REPLACE FUNCTION cht.deps_save_and_drop_dependencies(p_view_schema name, p_view_name name)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
declare
...
(
  ...
  insert into cht.deps_saved_ddl_temp(deps_view_schema, deps_view_name, deps_ddl_to_run)
